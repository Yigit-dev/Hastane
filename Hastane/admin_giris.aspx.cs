...
namespace IleriNet_Final2
{
    public partial class admin_giris : System.Web.UI.Page
    {
        ...
        {
            /***********************ADMIN GİRİŞ YAP BUTONU*********************************/
            con.Open();
            SqlCommand cmd = new SqlCommand("SELECT * FROM tbl_admin_kayit WHERE tc='" + txtKimlik.Text + "' and sifre='" + txtSifre.Text + "'", con);
            ...
            if (dr.Read())
            {
                ...
                Response.Redirect("admin_anasayfa.aspx");
            }
            else
            {
